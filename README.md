...
[![tests](https://img.shields.io/badge/tests-383-brightgreen.svg)](tests/test_frozen_invariants.py)
...
**Re-verified at `0f1dce3`**, the third full run of the night, after the page generator changed twice more (a phone breakpoint, an `h1`, a keyboard-operable explorer, a darker accent token). Same result: `make all` exits 0, all four diff surfaces empty — `git status --short`, `git diff --stat`, `git status --short results/`, `git diff --stat results/frozen/` — with **64** files under `results/` actually rewritten by the run. The one commit after it touches `app.py`, `docs/DECK.md` and the test file only; `git diff --name-only 0f1dce3..HEAD` matches nothing under `src/` or the `Makefile`, so the result transfers to `HEAD` unchanged.

**Re-verified again at `65d5b8a`**, after the page generator changed. All four outputs empty — `git status --short`, `git diff --stat`, `git status --short results/`, `git diff --stat results/frozen/` — with `make all` exiting 0, 59 files under `results/` actually rewritten by the run rather than left untouched, and the clone's own suite green at 356/356 for that commit. The three commits after it touch `README.md`, `docs/SUBMISSION.md` and the test file only; `git diff --name-only 65d5b8a..HEAD` matches nothing under `src/` or the `Makefile`, so no code that writes into `results/` has moved since. One honest note on how it was run: the 470 MB substrate had been moved off this machine's `data/raw`, so the clone was pointed at another copy — and that copy's four md5s were checked against the values published above before anything ran, rather than assumed. The first attempt at this check **failed at `make check` with "MISSING substrate" and produced an empty diff for that reason**, which is the same trap as a skipped test: an empty diff from a run that never happened looks exactly like a pass.
...
- **383 automated checks** fail the build if the words and the data stop agreeing. They have caught us five times, including once when we published a number with the wrong sign.
