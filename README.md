...
[![tests](https://img.shields.io/badge/tests-160-brightgreen.svg)](tests/test_frozen_invariants.py)
...
3. **Five evaluations, three of which could only come back negative.** The
...
Five evaluations. Three came back negative. All five are reported.
...
- **Three of our five evaluations came back negative.** All five are reported, including the one that clears its bar by only 0.026.
...
- **160 automated checks** fail the build if the words and the data stop agreeing. They have caught us five times, including once when we published a number with the wrong sign.
