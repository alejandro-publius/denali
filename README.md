...
[![tests](https://img.shields.io/badge/tests-309-brightgreen.svg)](tests/test_frozen_invariants.py)
...
- **Static page with every number injected from frozen files** — 47 values pass through a `V()` helper that records each source; a number that cannot be traced does not render
...
- **309 automated checks** fail the build if the words and the data stop agreeing. They have caught us five times, including once when we published a number with the wrong sign.
