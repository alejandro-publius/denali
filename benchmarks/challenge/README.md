...
## Where our own method places: fourth of four on the metric that matters

**`denali rerank` is entered as a contestant and it does not win.** Its size-aware
residual scores a Spearman delta of **+0.0113** over the baseline — statistically
...
"reuse the hit count you already have" entry beats both at 0.6633 and 0.80.

The worked example in `data/` is a **half** correction — it subtracts half as much
of the size term as `rerank` does — and it scores 0.6466 with top-10 0.80. So the
ordering is not "correcting is bad": it is that correcting *all the way* costs most
of the cross-screen signal, and correcting halfway costs almost none of it.

This is not a contradiction of what the tool claims, and it is worth being exact
about the difference. `rerank` answers "which entries in this ranking are least
justified by this screen's own data" — and this project's evaluation 6 already
found that **26% of the apparent cross-screen agreement is set size rather than
biology**. A method that removes size will therefore predict the second screen
worse, *because part of what makes the second screen predictable is the same
confound*. Predicting a replicate and being right are different targets, and this
board measures the first one.

What the board does show, and what we did not know before running it, is that the
full correction sits well off the efficient frontier for prediction. That is a real
cost, it is ours, and it is printed in the same type size as everything else. This
project has published seven negative results out of eleven; this is not the place
