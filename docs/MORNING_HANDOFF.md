...
biology.** Thirteen evaluations were run, nine came back negative, and all thirteen are
reported.

## 2. The thirteen results
...
- MCP server wired to a client: started over stdio from /tmp, 6 tools listed, 13 calls returned non-empty (2 lookups into our result, plus audit/rerank/baseline on the caller's own data and floor over the published atlas)
