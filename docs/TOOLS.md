...
| **Anthropic MCP** | ✅ `mcp 1.29.0` | n/a | `src/mcp_server.py` started over stdio **from `/tmp`, not the repo root**, 6 tools listed, 13 calls returned non-empty — both refusal paths, and `audit`/`rerank`/`baseline`/`floor` on caller-supplied data and on seven malformed or under-specified inputs. Run by `tests/test_mcp_stdio.py`, 27 checks | ships the result **and the tool** |
