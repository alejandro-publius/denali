...
| **Anthropic MCP** | ✅ `mcp 1.29.0` | n/a | `src/mcp_server.py` started over stdio **from `/tmp`, not the repo root**, 5 tools listed, 11 calls returned non-empty — both refusal paths, and `audit`/`rerank`/`baseline` on caller-supplied data and on five malformed or under-specified inputs. Run by `tests/test_mcp_stdio.py`, 22 checks | ships the result **and the tool** |
